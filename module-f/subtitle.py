"""Subtitle generation with Whisper (OpenAI / local).

Phase 2: added transcribe() + generate_srt() methods.
"""

from __future__ import annotations

import logging
import os
import subprocess
from pathlib import Path
from typing import Any

logger = logging.getLogger(__name__)

OUTPUT_DIR = Path(os.getenv("VIDEO_OUTPUT_DIR", "./output")).resolve()
WHISPER_MODEL = os.getenv("WHISPER_MODEL", "base")
WHISPER_LANGUAGE = os.getenv("WHISPER_LANGUAGE", "zh")


class SubtitleGenerator:
    """Whisper-based subtitle engine."""

    def __init__(self):
        OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        self._model = None  # lazy load

    async def transcribe(
        self,
        audio_path: str,
        language: str | None = None,
    ) -> dict[str, Any] | None:
        """Transcribe audio and return full result dict with segments.

        Returns: {"text": str, "segments": [{"start": float, "end": float, "text": str}, ...]}
        Returns None on failure.
        """
        try:
            import whisper

            model = self._load_model(whisper)
            result = model.transcribe(
                audio_path,
                language=language or WHISPER_LANGUAGE,
                verbose=False,
            )

            segments = [
                {
                    "start": seg["start"],
                    "end": seg["end"],
                    "text": seg["text"].strip(),
                }
                for seg in result.get("segments", [])
            ]

            return {
                "text": result.get("text", "").strip(),
                "segments": segments,
            }

        except ImportError:
            logger.warning("openai-whisper not installed — using mock transcript")
            return self._mock_transcript()
        except Exception as e:
            logger.warning("Whisper transcription failed: %s", e)
            return None

    async def generate(
        self,
        audio_path: str,
        output_path: str | None = None,
        format: str = "srt",
    ) -> str | None:
        """Generate subtitles from an audio file. Returns path to .srt file."""
        output_path = output_path or str(
            OUTPUT_DIR / f"{Path(audio_path).stem}.{format}"
        )

        result = await self.transcribe(audio_path)
        if result is None or not result.get("segments"):
            logger.warning("No transcription segments — using mock SRT")
            return self._mock_srt(audio_path, output_path)

        try:
            srt_content = self.generate_srt(result["segments"])
            with open(output_path, "w", encoding="utf-8") as f:
                f.write(srt_content)
            logger.info(
                "Subtitles generated: %s (%d segments)",
                output_path, len(result["segments"]),
            )
            return output_path
        except Exception as e:
            logger.exception("Subtitle generation failed: %s", e)
            return None

    def generate_srt(
        self,
        segments: list[dict[str, float | str]],
    ) -> str:
        """Generate SRT format string from segments list.

        Each segment: {"start": float, "end": float, "text": str}
        """
        lines: list[str] = []
        for i, seg in enumerate(segments, 1):
            start = _format_srt_time(float(seg["start"]))
            end = _format_srt_time(float(seg["end"]))
            text = str(seg.get("text", "")).strip()
            lines.append(f"{i}\n{start} --> {end}\n{text}\n")
        return "\n".join(lines)

    async def generate_from_video(
        self,
        video_path: str,
        output_path: str | None = None,
    ) -> str | None:
        """Extract audio from video and generate subtitles."""
        audio_path = str(OUTPUT_DIR / f"{Path(video_path).stem}_audio.wav")

        cmd = [
            "ffmpeg", "-y",
            "-i", video_path,
            "-vn",
            "-acodec", "pcm_s16le",
            "-ar", "16000",
            "-ac", "1",
            audio_path,
        ]
        try:
            subprocess.run(cmd, capture_output=True, timeout=120, check=True)
        except Exception:
            logger.warning("Audio extraction failed for %s", video_path)
            return None

        result = await self.generate(audio_path, output_path)

        if Path(audio_path).exists():
            Path(audio_path).unlink()

        return result

    # ---- internal -----------------------------------------------------

    def _load_model(self, whisper_module):
        if self._model is None:
            self._model = whisper_module.load_model(WHISPER_MODEL)
        return self._model

    def _mock_transcript(self) -> dict[str, Any]:
        """Return mock transcript for testing."""
        return {
            "text": "AI 资讯每周视频简报 为您带来最新的人工智能动态",
            "segments": [
                {"start": 0.0, "end": 3.5, "text": "AI 资讯每周视频简报"},
                {"start": 3.5, "end": 6.0, "text": "为您带来最新的人工智能动态"},
            ],
        }

    def _mock_srt(self, audio_path: str, output_path: str) -> str:
        """Create a minimal .srt for testing."""
        content = self.generate_srt(self._mock_transcript()["segments"])
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(content)
        return output_path


def _format_srt_time(seconds: float) -> str:
    """Format seconds to SRT time format: HH:MM:SS,mmm."""
    h = int(seconds // 3600)
    m = int((seconds % 3600) // 60)
    s = int(seconds % 60)
    ms = int((seconds - int(seconds)) * 1000)
    return f"{h:02d}:{m:02d}:{s:02d},{ms:03d}"
